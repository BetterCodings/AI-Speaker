...
# 음성 인식(듣기, STT)
def listen(recongnizer, audio):
    try:
        text = recongnizer.recognize_google(audio, language='ko')
        print('[사용자] '+ text)
        answer(text)
...
def answer(input_text):
    ...
    if '안녕' in input_text:
        ...
        answer_text = '오늘은 서울 기온은 20도 입니다. 맑은 하늘이 예상됩니다.'
...
speak('무엇을 도와드릴까요?')
stop_listening = r.listen_in_background(m, listen) #목소리가 들려오면 처리
